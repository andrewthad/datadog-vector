...
pub struct PgtableConfig {
    ...
    columns: Vec<String>,
    // x /// Reload the table when this query returns a newer timestamp
    // x /// than the last one that was seen. The result set must contain
    // x /// zero rows or one row, and it must contain a single timestamptz
    // x /// column. The name of the column does not matter.
    // x ///
    // x /// Example: SELECT max(w.updated) FROM widgets w;
    // x reload_when: Option<String>,
